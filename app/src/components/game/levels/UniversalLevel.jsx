...
// Універсальний компонент рівня
// Підтримує два типи питань:
// 1. Бінарний вибір (answer: "category1" або "category2")
// 2. Множинний вибір (options: [{id, text, isCorrect, explanation}])
export default function UniversalLevel({ levelId, onComplete, onExit, onNextLevel }) {
  ...
  // Показати пояснення після відповіді (для multiple choice)
  const [showExplanation, setShowExplanation] = useState(false);
  const [selectedOption, setSelectedOption] = useState(null);

  // Отримуємо дані рівня
    ...
    return (
      <div className="game-container">
        <div className="game-content" style={{ textAlign: 'center', color: 'white' }}>
          <h2>Рівень {levelId} не знайдено</h2>
          <button className="btn btn-secondary" onClick={onExit}>Повернутись</button>
        </div>
      </div>
    );
  }

  // Визначаємо тип питань: binary (answer) або multiple (options)
  const questionType = data.questions[0]?.options ? 'multiple' : 'binary';
  ...
  // Ініціалізація homework з ключами з data
  useEffect(() => {
    if (data.homework?.tasks) {
      const initialHomework = {};
      data.homework.tasks.forEach(task => {
        initialHomework[task.key] = '';
      });
  ...
  // Обробка відповіді для binary типу
  const handleBinaryAnswer = (answer) => {
    if (isProcessing) return;
    setIsProcessing(true);

    const isCorrect = currentQuestion.answer === answer;
    setLastAnswer({ isCorrect, correctAnswer: currentQuestion.answer });
    ...
    // Показуємо результат і переходимо до наступного питання
    setTimeout(() => {
      goToNextQuestion();
    }, 1500);
  };

  // Обробка відповіді для multiple типу
  ...
  // Отримуємо категорії для binary типу
  const getCategories = () => {
  ...
  const handleScreenClick = (e) => {
    if (e.target.closest('button') || e.target.closest('input') || e.target.closest('.option-btn') || e.target.closest('.answer-btn')) {
      return;
    }
    if (isDialoguePhase) {
      ...
      <div className="answer-buttons" style={{
        ...
        {categories.map((cat) => (
          <button
            ...
            onClick={() => handleBinaryAnswer(cat.key)}
            disabled={isProcessing}
            style={{
              padding: '15px 30px',
              background: isProcessing
                ? (lastAnswer?.correctAnswer === cat.key ? '#7ED321' :
                   (lastAnswer && lastAnswer.correctAnswer !== cat.key && currentQuestion.answer !== cat.key ? '#E85A5A' : cat.color))
                : cat.color,
              border: 'none',
              borderRadius: '15px',
              color: 'white',
  ...
  // Рендер підказки для binary типу
  const renderBinaryHint = () => {
    if (!isProcessing || !lastAnswer || questionType !== 'binary') return null;
    if (lastAnswer.isCorrect) return null;
    ...
    return (
      <div style={{
        ...
        <p style={{ margin: 0 }}>{currentQuestion.hint}</p>
      </div>
    );
  };
  ...
  const renderPhase = () => {
    switch (phase) {
      case PHASES.INTRO:
        return (
          <div className="game-content">
            <Character type={data.character} name={data.characterName} />
            <DialogueBox
              ref={dialogueRef}
        ...
        return (
          <div className="game-content">
            ...
            <Character
              ...
              emotion={lastAnswer?.isCorrect ? 'happy' : 'neutral'}
            />
      ...
      case PHASES.RETRY_PROMPT:
        return (
          <div className="game-content">
            <Character type={data.character} name={data.characterName} emotion="happy" />
      ...
      case PHASES.RESULT:
        ...
        return (
          <div className="game-content result-screen">
            ...
            {passed && stars >= 3 && BADGES[data.badge] && (
              <div className="badge-earned">
                <img src={BADGES[data.badge]} alt="Бейдж" className="badge-image" />
      ...
      case PHASES.OUTRO:
        return (
          <div className="game-content">
            <Character type={data.character} name={data.characterName} emotion="celebrating" />
            <DialogueBox
              ref={dialogueRef}
  ...
  // Отримуємо локацію для фону
  const locationKey = data.location;
  const backgroundImage = LOCATIONS[locationKey] || LOCATIONS.warehouse;
